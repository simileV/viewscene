...
namespace
{
	...
	//const char * SAMPLE_FILENAME = "teapot.fbx";
	//const char * SAMPLE_FILENAME = "zombii2.fbx";
    ...
    void LoadCacheRecursiveNode(FbxNode *pNode, FbxAnimLayer *pAnimLayer, bool pSupportVBO)
    {
		...
		//cout << "initialized lMeshCache 0  !!!!!!!!!!" << endl;

		if (lNodeAttribute)
        {
			//cout << "initialized lMeshCache 1  !!!!!!!!!!" << endl;

            // Bake mesh as VBO(vertex buffer object) into GPU.
            if (lNodeAttribute->GetAttributeType() == FbxNodeAttribute::eMesh)
            {
				//cout << "initialized lMeshCache 2  !!!!!!!!!!" << endl;

                FbxMesh * lMesh = pNode->GetMesh();

                if (pSupportVBO && lMesh && !lMesh->GetUserDataPtr())
                {
					//cout << "initialized lMeshCache 3  !!!!!!!!!!" << endl;

                    FbxAutoPtr<VBOMesh> lMeshCache(new VBOMesh);

                    if (lMeshCache->Initialize(lMesh))
                    {
						//cout << "initialized lMeshCache 4  !!!!!!!!!!" << endl;
        ...
        {
            LoadCacheRecursiveNode(pNode->GetChild(lChildIndex), pAnimLayer, pSupportVBO);
        }
    }

	// Bake node attributes and materials for this scene and load the textures.
	void LoadCacheRecursiveScene(FbxScene * pScene, FbxAnimLayer * pAnimLayer, const char * pFbxFileName, bool pSupportVBO)
	{
		...
		LoadCacheRecursiveNode(pScene->GetRootNode(), pAnimLayer, pSupportVBO);
	}

    // Unload the cache and release the memory under this node recursively.
    void UnloadCacheRecursiveNode(FbxNode * pNode)
    {
        ...
        for (int lChildIndex = 0; lChildIndex < lChildCount; ++lChildIndex)
        {
            UnloadCacheRecursiveNode(pNode->GetChild(lChildIndex));
        }
    }

	// Unload the cache and release the memory from this scene and release the textures in GPU
	void UnloadCacheRecursiveScene(FbxScene * pScene)
	{
		const int lTextureCount = pScene->GetTextureCount();
		...
		UnloadCacheRecursiveNode(pScene->GetRootNode());
	}
...
SceneContext::~SceneContext()
{
    ...
    if (mScene)
    {
        UnloadCacheRecursiveScene(mScene);
    }
...
bool SceneContext::LoadFile()
{
    ...
    if (mStatus == MUST_BE_LOADED)
    {
        if (mImporter->Import(mScene) == true)
        {
            ...
            {
				cout << "converting axis system" << endl;
            ...
            {
				cout << "converting unit system" << endl;
            ...
            LoadCacheRecursiveScene(mScene, mCurrentAnimLayer, mFileName, mSupportVBO);
