...
void InitializeLights(const FbxScene* pScene, const FbxTime & pTime, FbxPose* pPose)
{
	...
	LightCache::InitializeEnvironment(pScene->GetGlobalSettings().GetAmbientColor());
	...
	FbxPose* pPose)
{
	...
	else
	{
		...
		//glm::vec3 myVec(0.f, 1.f, 2.f);
		//cout << "myVec = " << glm::to_string(myVec) << endl;
	...
	FbxPose* pPose)
{
	...
	// on each vertex and store them in lClusterDeformation and lClusterWeight.
